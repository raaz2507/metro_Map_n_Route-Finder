export default {
	meta: {
		title: "मेट्रो मैप जनरेटर",

		ogTitle: "मेट्रो मैप और रूट फाइंडर",

		ogDescription:
			"सर्वश्रेष्ठ मेट्रो मार्ग, किराया, यात्रा समय और मध्यवर्ती स्टेशनों की जानकारी आसानी से प्राप्त करें।",

		twitterTitle: "मेट्रो मैप और रूट फाइंडर",

		twitterDescription:
			"सर्वश्रेष्ठ मेट्रो मार्ग, किराया, यात्रा समय और मध्यवर्ती स्टेशनों की जानकारी आसानी से प्राप्त करें।",
	},

	common: {
		about: "परिचय",

		help: "सहायता",

		settings: "सेटिंग्स",

		other: "अन्य",

		recent: "हालिया",

		search: "खोजें",

		reset: "रीसेट",

		from: "से",

		to: "तक",

		yes: "हाँ",

		no: "नहीं",

		loading: "लोड हो रहा है...",
	},

	home: {
		appName: "मेट्रो मैप जनरेटर",

		language: "भाषा",

		theme: "थीम",

		recentSearches: "हाल की खोजें",

		clearAll: "सभी हटाएँ",

		recent: "हालिया",

		mostUsed: "सबसे अधिक उपयोग",

		az: "अ-ज्ञ",

		noRecentJourney: "कोई हालिया यात्रा उपलब्ध नहीं है",

		routeFinder: "मार्ग खोजक",

		startStation: "से",

		endStation: "तक",

		startPlaceholder: "प्रारंभ स्टेशन दर्ज करें",

		endPlaceholder: "गंतव्य स्टेशन दर्ज करें",

		swapStations: "स्टेशन बदलें",

		findRoute: "मार्ग खोजें",

		resetForm: "रीसेट",

		priority: "प्राथमिकता",

		shortest: "सबसे छोटा मार्ग",

		lessInterchange: "कम इंटरचेंज",

		journeyDetails: "यात्रा विवरण",

		map: "मानचित्र",
	},

	themes: {
		light: "क्लासिक लाइट",

		dark: "स्लीक डार्क",

		cyberpunk: "नियॉन साइबरपंक",

		vintage: "विंटेज रेट्रो",

		mint: "फॉरेस्ट मिंट",

		ghibli: "जिब्ली नॉस्टैल्जिया",
	},

	"nav-header": {
        home: "🏠 होम",
        recharge: "💳 कार्ड रिचार्ज",
        others: "🗂️ अन्य",
        help: "❓ सहायता",
        about: "ℹ️ परिचय"
    },
    "nav-sidebar": {
        recent: "हालिया",
        map: "मानचित्र",
        setting: "सेटिंग्स"
    },
    "nav-floating": {
        alarm: "अलार्म",
        ticket: "टिकट"
    },

	footer: {
		footerNavigation: "फुटर नेविगेशन",

		about: "परिचय",

		bookTicket: "टिकट बुक करें",

		github: "गिटहब",

		appTitle: "मेट्रो मैप जनरेटर",

		qrDescription:
			"इस एप्लिकेशन को अपने मोबाइल पर तुरंत खोलने के लिए QR कोड स्कैन करें।",

		features: "विशेषताएँ",

		interactiveMetroMap: "इंटरैक्टिव मेट्रो मानचित्र",

		routeFinder: "मार्ग खोजक",

		fareCalculator: "किराया गणक",

		shortestRoutes: "सबसे छोटा और कम इंटरचेंज मार्ग",

		mobileFriendly: "मोबाइल अनुकूल डिज़ाइन",

		brandTitle: "मेट्रो मैप जनरेटर",

		brandDescription: "शहरी परिवहन को और स्मार्ट बनाने के लिए विकसित।",

		disclaimer:
			"मार्ग, किराया और यात्रा संबंधी जानकारी केवल संदर्भ हेतु प्रदान की जाती है।",

		developedBy: "❤️ के साथ विकसित",
	},

	route: {
		stations: "{count} स्टेशन",
		station: "{count} स्टेशन",
		fare: "₹{fare}",
		travelTime: "{minutes} मिनट",
		distance: "{distance} किमी",
		interchange: "{count} इंटरचेंज",
		interchanges: "{count} इंटरचेंज",
        warning: "दिखाया गया समय केवल अनुमानित यात्रा समय है। यात्रियों को यात्रा के लिए अतिरिक्त समय रखने की सलाह दी जाती है।",
        showOnMap: "मैप पर मार्ग दिखाएं",
        gate: "गेट",
        first: "पहली ट्रेन",
        last: "आखिरी ट्रेन",
        minutesLabel: "मिनट",
        lineChangeLabel: "लाइन बदलाव",
        stationsLabel: "स्टेशन",
        tokenFareLabel: "किराया",
        directionText: "दिशा: {terminal} की ओर (प्लेटफ़ॉर्म नंबर {platform})",
        changeToText: "<strong>{line}</strong> पर बदलें, <strong>{terminal}</strong> की ओर (प्लेटफ़ॉर्म नंबर {platform})"
	},
};
