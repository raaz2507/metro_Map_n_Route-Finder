...
export class StationInfoManager {
    ...
    let prevStackHTML = '';
    let nextStackHTML = '';
    const currentName = stBasic?.name?.en || this.stationId.toUpperCase();

    lineKeys.forEach(lineId => {
        const lineInfo = this.metroLines[lineId];
        const lineColor = lineInfo?.color || '#007bff';

        // 👈 सीधे stBasic.neighbors से इस लाइन के 2 पड़ोसी (prev और next) निकालें
        const lineNeighbors = neighbors.filter(n => n.line === lineId);
        
        const prevNeighbor = lineNeighbors[0];
        const nextNeighbor = lineNeighbors[1];

        const prevName = prevNeighbor ? (this.metroStations[prevNeighbor.station]?.name?.en || prevNeighbor.station) : 'Terminal';
        const nextName = nextNeighbor ? (this.metroStations[nextNeighbor.station]?.name?.en || nextNeighbor.station) : 'Terminal';
    ...
    container.innerHTML = `
        <div class="track-col prev-col">
            ${prevStackHTML}
        </div>
        <div class="track-col center-col">
            ${currentHTML}
        </div>
        <div class="track-col next-col">
            ${nextStackHTML}
        </div>
    `;
}
    ...
    renderLineInfoCard(stBasic, stRich) {
        ...
        const description = stRich?.description || '';
        const isWalkway = stBasic?.properties?.station_type === 'walkway' || 
                description.toLowerCase().includes('walkway') ||
                               description.toLowerCase().includes('travelator');
             if (isWalkway) {
                 linesHTML += `
                     <div class="line-strip-row" style="margin-bottom: 0.4rem;">
                         <span class="interchange-badge-chip">🚶 Interchange via Walkway / Travelators</span>
                     </div>
                 `;
             }
        ...
        lineKeys.forEach(lineId => {
            ...
            const colorName = lineInfo.color_name 
                ? `${lineInfo.color_name.toUpperCase()} LINE` 
                : (lineInfo.name?.en ? lineInfo.name.en.split('-')[1]?.trim() : lineId.toUpperCase());
            ...
            const fromSt = fromId ? (this.metroStations[fromId]?.name?.en || fromId.toUpperCase()) : 'Origin';
            const toSt = toId ? (this.metroStations[toId]?.name?.en || toId.toUpperCase()) : 'Destination';
            linesHTML += `
        ...
        linesHTML += '</div>';
        container.innerHTML = `
            <h3 class="feature-card-header">🚆 Connected Metro Lines</h3>
            ${linesHTML}
        `;
    }
    ...
    renderHero(stBasic, stRich) {
        ...
        const nameEn = stBasic?.name?.en || stRich?.id?.toUpperCase() || this.stationId.toUpperCase();
        const nameHi = stBasic?.name?.hi ? `(${stBasic.name.hi})` : '';
        ...
        heroContainer.innerHTML = `
            <div class="station-hero-header">
                <div class="station-main-title">
                    <h2>${nameEn} <span style="font-weight:normal; font-size:0.7em; opacity:0.8;">${nameHi}</span></h2>
    ...
    renderTimings(stBasic, stRich) {
        ...
        const opening = this.formatTo12Hour(stRich?.timings?.opening);
        const closing = this.formatTo12Hour(stRich?.timings?.closing);
        ...
        container.innerHTML = `
            <div class="compact-timing-row">
                <span>⏰ <strong>Station Hours:
            </div>
            <div class="compact-timing-row border-top-dashed">
                <span>☀️ <strong>First:</strong> <span style="color:rgb(var(--color-success-rgb)); font-weight:700;">${firstTrain}</span></span>
                <span>🌙 <strong>Last:</strong> <span style="color:rgb(var(--color-danger-rgb)); font-weight:700;">${lastTrain}</span></span>
            </div>
        `;
    }
    ...
    renderHelplines(stRich) {
        ...
        container.innerHTML = `
            <h3 class="feature-card-header">📞 Helplines & Support</h3>
            <div class="contacts-grid">
                <div class="contact-box">
                    <strong>Station Mobile Helpline</strong>
                    <div><a href="tel:${mobile}">${mobile}</a></div>
                </div>
                <div class="contact-box">
                    <strong>Station Landline</strong>
                    <div style="font-weight:700; color:var(--btn-primary-bg); margin-top:3px;">${landline}</div>
                </div>
                <div class="contact-box">
                    <strong>DMRC Universal Helpline</strong>
    ...
    ? `<span class="divyang-tag accessible">♿ Divyang Accessible</span>` 
    : `<span class="divyang-tag standard">🚶 Standard Access</span>`;
const gateCode = g.code || `GA${gNum}`;
            const status = (g.status || 'OPEN').toUpperCase();
            const landmark = g.landmark?.en || g.landmark?.hi || 'Exit Gate Area';
    ...
    if (liftKeys.length > 0) {
        finalHTML += `
            <div class="facility-subcategory">
                <div class="facility-subcategory-title">🛗 Lifts (${liftKeys.length})</div>
                <div class="table-responsive-wrapper">
                    <table class="station-table">
                        <thead>
                            <tr>
                            ...
                            ${liftKeys.map(k => {
                                ...
                                return `
                                    <tr>
                                        ...
                                        <td>${item.divyang_friendly ? '♿ Divyang Friendly' : 'Standard'}</td>
                                        <td class="${item.status !== false ? 'status-active' : 'status-maintenance'}">${item.status !== false ? 'Active' : 'Maintenance'}</td>
                                    </tr>
                                `;
                            }).join('')}
                        </tbody>
    ...
    if (escKeys.length > 0) {
        finalHTML += `
            <div class="facility-subcategory">
                <div class="facility-subcategory-title">🪜 Escalators (${escKeys.length})</div>
                <div class="table-responsive-wrapper">
                    <table class="station-table">
                        <thead>
                            <tr>
        ...
        if (main) {
            main.innerHTML = `
                <div style="text-align:center; padding:5rem 1rem; color:var(--text-secondary);">
                    ...
                    <a href="all_stations.html" class="back-btn" style="margin-top:1.5rem;">⬅️ Return to All Stations Directory</a>
                </div>
            `;
        }
    }
