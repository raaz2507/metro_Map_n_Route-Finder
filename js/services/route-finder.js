import { getDistance } from "../core/data-utils.js";
...
export class RouteFinder {
	...
	/**
	 * पाथ (Path) के आधार पर सेगमेंट लाइन्स और ग्रूप किए गए सेगमेंट्स निकालता है।
	 * @param {Array<string>} path 
	 * @returns {{ segmentLines: Array<string>, segments: Array<Object> }}
	 */
	getRouteSegments(path) {
		if (!path || path.length === 0) return { segmentLines: [], segments: [] };

		const getSegmentLine = (s1Id, s2Id) => {
			const s1 = this.#stationData[s1Id];
		...
		return { segmentLines, segments };
	}
	/**
	 * स्टेशन पाथ और लाइन ID के आधार पर अंतिम (Terminal) स्टेशन ID ढूंढता है।
	...
	getTerminalStationId(stationIds, lineId) {
		...
		for (let iter = 0; iter < 100; iter++) {
			const currentStation = this.#stationData?.[currentId];
	...
	/**
	 * इंटरचेंज स्टेशन के लिए target line और terminal station id के आधार पर प्लेटफॉर्म नंबर बताता है।
	 * @param {string|Object} stationOrId 
	 * @param {string} targetLineId 
	 * @param {string} terminalStationId 
	 * @returns {string}
	 */
